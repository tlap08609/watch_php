		...
		<div class="row"><div class="col-md-12">
			<form action="usr.php" method="post">
				<div class="panel panel-info"><div class=panel-heading><h3 class=panel-title>會員管理</h3></div><div class=panel-body>
					<div class=col-sm-3><div class=form-group><label for=s>會員狀態</label><select class=form-control id="s" name="s"><option>上線會員</option><option>禁用會員</option></select></div></div>
					<div class=col-sm-3><div class=form-group><label for=i>帳號</label><input class=form-control id=i></div></div>
					<div class=col-sm-3><div class=form-group><label for=e>信箱</label><input class=form-control id=e name=e></div></div>
			...
			if ($result->num_rows > 0) {?>
				<table class='table table-bordered table-hover'>
					...
					<?php if ($row["status"]==1) {?><button class="btn btn-danger" name="d" value="<?php echo $row['uid']?>">停用</button><?php } else{?><button class="btn btn-info" name="d" value="<?php echo $row['uid']?>">啟用</button><?php }?></div></td></tr>
					<?php 
 					}?>
 				</table><?php 
 			} else {?>
